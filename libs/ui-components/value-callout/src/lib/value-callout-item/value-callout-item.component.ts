...
import { Component, OnInit, Directive, ViewEncapsulation, ElementRef } from '@angular/core';
import { mixinTextColor, HasColor, ComponentHostBase } from '@hza/ui-components/core';

export const _DashboardOverviewValueCalloutBase = mixinTextColor(ComponentHostBase);

@Component({
  selector: 'fs-value-callout-item',
...
export class DashboardOverviewValueCalloutComponent extends _DashboardOverviewValueCalloutBase
  ...
  ngOnInit() {
    this.classes = `fs-value-callout-item-main-${this.displayType}`;
  }
}

@Directive({
  selector: 'fs-value-callout-item-title, [fsValueCalloutItemTitle]',
  host: { class: 'fs-value-callout-item-title' },
})
