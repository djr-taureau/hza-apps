...
import { Directive, Input, ElementRef, AfterContentInit, Renderer2 } from '@angular/core';
...
export class InlineCaretDirective implements AfterContentInit {
  ...
  createCaret(direction: CaretDirection, color: CaretColor, position: CaretPosition, size: CaretSize) {
  ...
  insertCaret(
    ...
    if (location === 'start') {
      this._renderer.insertBefore(this.getHostElement(), caret, this.getHostElement().firstChild);
    } else {
  ...
  ngAfterContentInit() {
    this.insertCaret(this.caretDirection, this.caretLocation, this.caretColor, this.caretPosition, this.caretSize);
  }
}
