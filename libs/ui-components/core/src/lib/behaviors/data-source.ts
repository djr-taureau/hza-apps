...
export interface Datasource<T> {
	...
	raiseItemEvent(action: DataSourceItemAction, item?: DatasourceItem<T>, $event?: MouseEvent);
}

export function mixinDataSource<T extends Constructor<{}>, Y>(base: T): Constructor<Datasource<Y>> & T {
	return class extends base {
		items: DatasourceItems<Y>;
		event: EventEmitter<DatasourceItemEvent<Y>> = new EventEmitter();

		raiseItemEvent(action: DataSourceItemAction, item: DatasourceItem<Y>, $event: MouseEvent) {
