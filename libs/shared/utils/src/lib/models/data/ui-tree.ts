...
export class FlatDataNode<T> extends DataNode<T> {
  ...
  constructor(public display: string, public data: T, _expandable: boolean, public level: number) {
    super(display, data);
...
export class UITreeControl<T> extends FlatTreeControl<FlatDataNode<T>> {
  constructor() {
    super((node: FlatDataNode<T>) => node.level, (node: FlatDataNode<T>) => node.expandable);
  }
}

export class UITreeFlattener<T> extends MatTreeFlattener<FlatDataNode<T>, FlatDataNode<T>> {
  constructor() {
    super(
      (node: FlatDataNode<T>, level: number) =>
        new FlatDataNode<T>(node.display, node.data, node.children && node.children.length > 0, level),
      (node: FlatDataNode<T>) => node.level,
      (node: FlatDataNode<T>) => node.expandable,
      (node: FlatDataNode<T>): Observable<FlatDataNode<T>[]> => observableOf(node.children),
    );
  }
}

export class UITreeDataSource<T> extends MatTreeFlatDataSource<FlatDataNode<T>, FlatDataNode<T>> {
...
export function buildDataNodeTree<T, N extends FlatDataNode<T>, S>(
  ...
  const mapTo = (source: S, level: number) =>
    Object.keys(source).reduce<Array<N>>((accumulator, key) => {
