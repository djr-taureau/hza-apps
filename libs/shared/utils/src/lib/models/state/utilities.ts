 ...
 */
export const setEntityProp = (...props: string[]) => <V, S>(id: string, value: V, state: S) =>
  assocPath<V, S>(['entities', id, ...props], value, state) as S;

/**
 * Updates an Entity in state
 */
export const setEntity = <V, S>(id: string, value: V, state: S) => assocPath<V, S>(['entities', id], value, state) as S;
 ...
 */
export const setDataState = <SetDataStateFn>pipe(
  setEntities,
  setLoadedState,
);
