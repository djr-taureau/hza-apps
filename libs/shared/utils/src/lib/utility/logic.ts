import { isNil, isEmpty, either, complement, equals, test, when, prop, pipe } from 'ramda';
 ...
 */
const buildWhenFn = <T>(pred: TypedPredicate<T>) => <U>(thenFn: (a: T) => U) => when<T, U>(pred, thenFn);
 ...
 */
export function whenPropIsNotUseable<T, U>(property: string, thenFn: (a: T) => U) {
  return pipe<T, any, U>(
    prop(property),
